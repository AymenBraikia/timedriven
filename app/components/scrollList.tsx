	...
	return (
		<div className="relative w-full h-full overflow-hidden flex items-center py-20">
			<div
				...
				className={`flex  w-full ${isTransitioning ? "transition-transform duration-300 ease-in-out" : ""}`}
				style={{
					transform: `translateX(-${swipe * (100 / display)}%)`,
				}}
			>
				{extendedItems.map((child, i) => (
					<div key={i} className="shrink-0 px-2" style={{ width: `${100 / display}%` }}>
						{child}
					</div>
				))}
			</div>
			...
			<div className="absolute left-1/2 -translate-x-1/2 bottom-0 flex-center gap-4">
				<button className={`cursor-pointer rotate-180 p-2 z-10 ${!canLoop ? "opacity-50 cursor-not-allowed" : ""}`} onClick={handlePrev} disabled={!canLoop}>
					<Next s={40} clr="currentColor" />
				</button>
				{gen_dot(React.Children.count(children), index)}

				<button className={`cursor-pointer p-2 z-10 ${!canLoop ? "opacity-50 cursor-not-allowed" : ""}`} onClick={handleNext} disabled={!canLoop}>
					<Next s={40} clr="currentColor" />
				</button>
			</div>
		</div>
