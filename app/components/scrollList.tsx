...
// Define the responsive breakpoints structure
...
export default function List({ children, display }: { children: React.ReactNode; display: number | Breakpoints }) {
	...
	// 1. Track the active display count based on the window width
	const [currentDisplay, setCurrentDisplay] = useState<number>(() => {
		if (typeof display === "number") return display;
		return display.base || 1; // Default SSR safe fallback
	});
	...
	useEffect(() => {
		...
		handleResize(); // Catch client size immediately on mount
	...
	// 2. Smoothly reposition the layout when the responsive screen sizes change
	useEffect(() => {
	...
	return (
		<div className="relative w-full h-full overflow-hidden flex items-center py-20">
			...
			<div className="absolute left-1/2 -translate-x-1/2 bottom-0 flex items-center gap-4">
