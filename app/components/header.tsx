	...
	return (
		<header className={`flex justify-between items-center w-dvw fixed top-0 left-0 z-50 p-8 transition-default ${ui.isGlassy ? "h-25 sm:h-30 text-primary" : "h-20 sm:h-25 text-white"}`}>
			...
			<div className={`w-1/3 flex justify-start items-center transition-default ${ui.isNavOpen ? "opacity-0" : "opacity-100"}`}>
				<button type="button" className="button2 float-anim" onClick={() => dispatch({ type: "OPEN_NAV" })}>
			...
			<div className="flex justify-end items-center gap-2 w-1/3">
				...
				<button type="button" className={`button2 hidden sm:block ${ui.isGlassy ? "" : "hover:text-primary"}`} onClick={() => dispatch({ type: "TOGGLE_LANG" })}>
					{ui.lang}
				</button>
				<button type="button" onClick={() => dispatch({ type: "OPEN_CART" })} className={`button2 ${ui.isGlassy ? "" : "hover:text-primary"}`}>
			...
			<nav className={`liquid-glass w-dvw md:w-[20dvw] transition-default h-dvh fixed top-0 left-0 flex flex-col justify-start items-start gap-16 p-16 ${ui.isNavOpen ? "translate-x-0" : "-translate-x-full"}`}>
				...
				<button type="button" className="absolute top-4 right-4 p-0 cursor-pointer" onClick={() => dispatch({ type: "CLOSE_NAV" })}>
			...
			<button
				type="button"
				className={`fixed left-0 top-0 w-dvw h-dvh bg-black/50 cursor-default text-left transition-default ${ui.isCartOpen ? "fade-in z-10 block" : "fade-out hidden"}`}
				onClick={(e) => e.target === e.currentTarget && dispatch({ type: "CLOSE_CART" })}
			>
				<div className="liquid-glass w-dvw md:w-[30dvw] h-dvh fixed top-0 right-0 flex flex-col justify-start items-start gap-16 p-16 font-secondary" onClick={(e) => e.stopPropagation()}>
					<div className="flex justify-between items-center w-full h-1/10">
						<h3 className="title3">YOUR COLLECTION</h3>
						<button type="button" className="button2 p-1" onClick={() => dispatch({ type: "CLOSE_CART" })}>
							<Cross classnames={"w-16"} />
						</button>
					</div>
					...
					<div className="flex flex-wrap justify-between items-center gap-4 w-full h-1/10">
						...
						<button type="button" className="button w-full flex-center title5">
