    ...
    return (
        <>
            <header className="flex flex-wrap items-end justify-between gap-4 mb-6">
                <div>
                    <h1 className="text-2xl font-[family-name:var(--font-gelasio)]">Watches</h1>
                ...
                <Link href="/admin/watches/new" className="px-5 py-2.5 bg-[var(--foreground)] text-[var(--background)] text-sm">
                    Add a watch
                </Link>
            </header>
            ...
            <form className="mb-6">
                <input
                    ...
                    className="w-full max-w-md outline-none text-base border-b border-[var(--foreground)]/30 py-1 bg-transparent focus:border-[var(--foreground)] transition-colors duration-300"
                />
            </form>
            ...
            {watches.length === 0 ? (
                ...
                <ul className="flex flex-col divide-y divide-[var(--foreground)]/10">
                    {watches.map((watch) => (
                        <li key={watch.slug} className="flex items-center gap-4 py-3">
                            <div className="relative w-14 h-14 shrink-0 bg-[var(--bg-secondary)]">
                                {watch.images[0] && <Image src={watch.images[0]} alt="" fill sizes="56px" quality={60} className="object-cover" />}
                            </div>
